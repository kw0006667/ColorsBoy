Shader "Hidden/BlendForBloom" {
	...
	half _ChromaticBloom;
	half _Intensity;
	half _ChromaticBloomThresh;
	...
	half4 fragScreen (v2f i) : COLOR {
		...
		screencolor.g = lerp(screencolor.g, tex2D(_ColorBuffer, i.uv[1] + _ColorBuffer_TexelSize * half2(-1,1) * 0.5 * step(_ChromaticBloomThresh, Luminance(screencolor.rgb)) ).g, _ChromaticBloom);
...
Subshader {
 ...
 // 1: "add" blend mode (with chromatic)
 Pass {    
