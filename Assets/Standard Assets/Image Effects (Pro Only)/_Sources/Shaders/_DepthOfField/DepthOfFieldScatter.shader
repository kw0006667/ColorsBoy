 Shader "Hidden/Dof/DepthOfFieldHdr" {
	...
	float4 fragBlurGauss (v2f i) : COLOR 
	{
		const int TAPS = 12;

		float4 centerTap = tex2D(_MainTex, i.uv1.xy);

		// TODO: important ? braks when HR blur is being used
	...
	float4 fragVisualize (v2f i) : COLOR 
	{
		...
		returnValue.rgb = lerp(returnValue.rgb, float3(1,1,1), returnValue.a/_CurveParams.x);
		return returnValue;
	}
	...
	float4 fragBlurAlphaWeighted (v2fBlur i) : COLOR 
	{
		...
		w = saturate(10.0*sampleB.a) * G_WEIGHTS[1]; sum += sampleB * w; weights += w;
		w = saturate(10.0*sampleC.a) * G_WEIGHTS[1]; sum += sampleC * w; weights += w;
		w = saturate(10.0*sampleD.a) * G_WEIGHTS[2]; sum += sampleD * w; weights += w;
		w = saturate(10.0*sampleE.a) * G_WEIGHTS[2]; sum += sampleE * w; weights += w;
		w = saturate(10.0*sampleF.a) * G_WEIGHTS[3]; sum += sampleF * w; weights += w;
		w = saturate(10.0*sampleG.a) * G_WEIGHTS[3]; sum += sampleG * w; weights += w;
		w = saturate(10.0*sampleH.a) * G_WEIGHTS[4]; sum += sampleH * w; weights += w;
		w = saturate(10.0*sampleI.a) * G_WEIGHTS[4]; sum += sampleI * w; weights += w;
		w = saturate(10.0*sampleJ.a) * G_WEIGHTS[5]; sum += sampleJ * w; weights += w;
		w = saturate(10.0*sampleK.a) * G_WEIGHTS[5]; sum += sampleK * w; weights += w;
